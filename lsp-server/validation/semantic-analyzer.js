...
class SemanticAnalyzer {
    ...
    validateFirstSyllable(firstSyllable) {
    ...
    validateSyllables(syllables, headers) {
    ...
    validateNoteGroup(noteGroup, hasNabcLines, previousSyllable) {
    ...
    validateNABCAlternation(syllables, headers) {
    ...
    validateMusicalConstructions(noteGroup, previousSyllable) {
        ...
        for (let i = 0; i < notes.length; i++) {
            ...
            const hasQuadratum = note.modifiers?.some(m => m.type === types_1.ModifierType.Quadratum);
            const hasFusion = note.modifiers?.some(m => m.type === types_1.ModifierType.Fusion);
            ...
            const noteFusion = note.modifiers?.some(m => m.type === types_1.ModifierType.Fusion);
            if (note.shape === types_1.NoteShape.Quilisma && !nextNote && !noteFusion) {
                this.warnings.push({
            ...
            const hasOriscusScapus = note.modifiers?.some(m => m.type === types_1.ModifierType.OriscusScapus);
            const prevNoteFusion = prevNote?.modifiers?.some(m => m.type === types_1.ModifierType.Fusion);
            const currentNoteFusion = note.modifiers?.some(m => m.type === types_1.ModifierType.Fusion);
            if (hasOriscusScapus) {
            ...
            if (note.shape === types_1.NoteShape.Quilisma && nextNote) {
            ...
            if (note.shape === types_1.NoteShape.Virga && this.hasStrataModifier(note) && nextNote) {
                if (this.comparePitch(nextNote.pitch, note.pitch) >= 0) {
                    this.warnings.push({
            ...
            if (note.shape === types_1.NoteShape.Punctum && this.hasStrataModifier(note) && nextNote) {
    ...
    checkQuilismaticConnector(noteGroup) {
        const notes = noteGroup.notes;
        // Check for quilismatic sequences of 3+ notes
        if (notes.length >= 3) {
            for (let i = 0; i < notes.length; i++) {
                const note = notes[i];
                if (note.shape === types_1.NoteShape.Quilisma) {
    ...
    formatNoteSequence(notes, quilismaIndex) {
 ...
 */
function analyzeSemantics(document) {

...
function getTokenTypeName(typeIndex) {
    return semanticTokensProvider_1.tokenTypes[typeIndex] || 'unknown';
}
function printToken(token, text) {
...
suite('Semantic Tokens Test Suite', () => {
    let provider;
    setup(() => {
        provider = new semanticTokensProvider_1.GabcSemanticTokensProvider();
    });
...
(c4) Test(e|ta|f|vi)`;
        ...
        if (result) {
            ...
            const pitchTokens = notationTokens.filter(t => getTokenTypeName(t.type) === 'variable' || getTokenTypeName(t.type) === 'type');
...
(c4) Test(gh|pe|f|ta|e|vi)`;
        ...
        if (result) {
            ...
            pitchTokens.forEach((token, index) => {
                const content = notationLine.substring(token.char, token.char + token.length);
                assert.strictEqual(content, expectedPitches[index], `Pitch ${index} should be '${expectedPitches[index]}', got '${content}'`);
                console.log(`  ✓ Pitch ${index}: '${content}' at position ${token.char}`);
            });
            const expectedNabc = ['pe', 'ta', 'vi'];
            nabcTokens.forEach((token, index) => {
                const content = notationLine.substring(token.char, token.char + token.length);
                assert.strictEqual(content, expectedNabc[index], `NABC ${index} should be '${expectedNabc[index]}', got '${content}'`);
...
(c4) Test(e|ta|f|vi)`;
        ...
        if (syllable && syllable.notes.length > 0) {
            ...
            noteGroup.notes.forEach((note, index) => {
                ...
                assert.ok(noteContent.includes(note.pitch), `Note content should include pitch "${note.pitch}", got "${noteContent}"`);
            });
        }
    });
...
(c4) Foo(e|ta|un) Bar(f|vi|un)`;
        ...
        const provider = new semanticTokensProvider_1.GabcSemanticTokensProvider();
        const tokens = provider.provideDocumentSemanticTokens(doc, new vscode.CancellationTokenSource().token);
